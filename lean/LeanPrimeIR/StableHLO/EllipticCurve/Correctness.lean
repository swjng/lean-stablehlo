...
theorem addY_eq_mathlib
    ...
    (x₁ y₁ x₂ ℓ : Expr p) :
    (LeanPrimeIR.StableHLO.addY x₁ y₁
      (LeanPrimeIR.StableHLO.addX x₁ x₂ ℓ) ℓ).eval =
...
theorem addDistinct_matches_mathlib
  ...
  refine ⟨?_, ?_⟩
  · show (LeanPrimeIR.StableHLO.addX x₁ x₂ (slopeAdd x₁ y₁ x₂ y₂)).eval = _
    rw [hax, hsl]
  · show (LeanPrimeIR.StableHLO.addY x₁ y₁
      (LeanPrimeIR.StableHLO.addX x₁ x₂ (slopeAdd x₁ y₁ x₂ y₂))
...
theorem double_matches_mathlib
  ...
  · -- x-coordinate: expose reduced form for rw
    show (LeanPrimeIR.StableHLO.addX x₁ x₁ (slopeDouble x₁ y₁)).eval = _
    rw [hax, hsl]
  · -- y-coordinate: expose reduced form for rw
    show (LeanPrimeIR.StableHLO.addY x₁ y₁
      (LeanPrimeIR.StableHLO.addX x₁ x₁ (slopeDouble x₁ y₁))
